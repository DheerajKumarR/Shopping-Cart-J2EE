...
import com.shop.dto.OrderService;
...
public class OrderServiceImpl implements OrderService {
	...
	public String paymentSuccess(String userName, double paidAmount) {
		...
		List<CartDao> cartItems = new ArrayList<CartDao>();
		cartItems = new CartServiceImpl().getAllCartItems(userName);

		if (cartItems.size() == 0)
			return status;

		TransactionDao transaction = new TransactionDao(userName, paidAmount);
		...
		for (CartDao item : cartItems) {

			double amount = new ProductServiceImpl().getProductPrice(item.getProdId()) * item.getQuantity();

			OrderDao order = new OrderDao(transactionId, item.getProdId(), item.getQuantity(), amount);
	...
	public List<OrderDao> getAllOrders() {
		...
		try {
			...
			while (rs.next()) {
				...
				OrderDao order = new OrderDao(rs.getString("orderid"), rs.getString("prodid"), rs.getInt("quantity"),
	...
	public List<OrderDao> getOrdersByUserId(String emailId) {
		...
		try {
			...
			while (rs.next()) {
				...
				OrderDao order = new OrderDao(rs.getString("t.transid"), rs.getString("t.prodid"),
